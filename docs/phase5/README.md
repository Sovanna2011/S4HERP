...
| Posting engine (381 checks) | The §23 rules the engine enforces, over HTTP, including the approval workflow | `db/tests/posting-engine.sh` |
| Browser acceptance (63 checks) | The UI journeys, in Chromium against the real API | `ui5/test/ui-acceptance.mjs` |

**452 automated checks**, all passing. Every one asserts a *specific* outcome —
...
./db/tests/posting-engine.sh                      # 381 posting-engine checks
