...
| Posting engine (227 checks) | The §23 rules the engine enforces, over HTTP, including the approval workflow | `db/tests/posting-engine.sh` |
| Browser acceptance (27 checks) | The UI journeys, in Chromium against the real API | `ui5/test/ui-acceptance.mjs` |

**273 automated checks**, all passing. Every one asserts a *specific* outcome —
...
./db/tests/posting-engine.sh                      # 227 posting-engine checks
